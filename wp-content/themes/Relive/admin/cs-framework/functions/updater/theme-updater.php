...
$updater = new Themedd_Updater_Admin(
	...
	// Config settings
	$config = array(
		'remote_api_url' => 'https://wpmes.cn',
		'item_name'      => 'Relive',
		'theme_slug'     => 'Relive',
		'version'        => wp_get_theme()->get( 'Version' ),
		'author'         => wp_get_theme()->get( 'Author' ),
		'download_id'    => '14052', // Optional, used for generating a license renewal link
...
function xintheme_license_notify() {
    $status = get_option( 'relive_license_key_status' );
    ...
    if( !is_admin() ){
        wp_die(
			...
			<div class="no-verify-inner" style="background: #fff6dc;padding: 20px;margin-top: 20px;">
				<p style="font-family:microsoft yahei;margin:10px 0;line-height:30px">您好，您还没有激活本主题， 请到 <a href="/wp-admin/themes.php?page=relive-license">[后台 → 外观 → 主题授权 ]</a>进行激活</br>
			</p>
			</div>
			</div>'
        );
