...
if($_POST['action']){ 
	...
	if($action == 'XinTheme_login'){
	...
	}elseif($action == 'XinTheme_register'){
		...
		if ( $sanitized_user_login == '' ) {
		  ...
		  if(empty($_POST['captcha']) || empty($_SESSION['XinTheme_captcha']) || trim(strtolower($_POST['captcha'])) != $_SESSION['XinTheme_captcha']){
		  	  $error .= '验证码错误 ';
		  }
		  
		  if($_SESSION['XinTheme_captcha_email'] != $user_email){
		  	  $error .= '验证码与邮箱不对应 ';
		  }
		  
		  if($error){ echo $error;}
		  else{
		  	unset($_SESSION['XinTheme_captcha']);
			unset($_SESSION['XinTheme_captcha_email']);
