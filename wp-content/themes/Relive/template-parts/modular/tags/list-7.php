...
<div class="<?php if( $id['modular_list_mobile'] ){ ?>mobile_modular_no <?php } ?>mnmd-block<?php if( $full_background_white ){?> relive_v3<?php }?>">
	<div class="row row--space-between">
		<?php
		$tag_id = explode(",",$id['modular_tags_list']);
		foreach ( $tag_id as $tag_ids ){
		?>
		<div class="col-xs-12 col-sm-6">
			<div class="block-heading block-heading--line">
				<h4 class="block-heading__title"><?php $cat = get_tag($tag_ids);echo $cat->name; ?></h4>
			</div>
			<?php
			...
			<ul class="list-space-xs list-square-bullet-exclude-first list-seperated-exclude-first list-unstyled">
				<?php elseif ( 2 === $i ) : ?>
				<li><article class="post post--horizontal post--horizontal-xs">
				<div class="post__thumb min-height-75">
					<a href="<?php the_permalink(); ?>" alt="<?php the_title(); ?>">
						<img <?php echo is_lazysizes(); ?>src="<?php echo post_thumbnail(180, 135); ?>" alt="<?php the_title(); ?>" width="180" height="135">
					</a>
				</div>
